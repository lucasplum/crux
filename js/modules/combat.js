...
var combatStats = {
  totalRounds: 0, totalDamage: 0, kills: 0, crits: 0, misses: 0,
...
function syncPlayerData() {
  ...
  combatants.forEach(function(c) {
    var player = players.find(function(p) { return p.name === c.name && p.role === c.role; });
    if (player) {
...
function openAddFromPartyPopup() {
  ...
  var listItems = players.map(function(p, i) {
    ...
    return '<div class="party-list-item">' +
  ...
  popup.innerHTML = '<div class="popup-content" style="max-width:520px;">' +
    ...
    '<div class="party-select-all">' +
      ...
      '<span style="font-size:0.7rem;color:var(--parchment-dim);">Tylko żywi będą dodani</span>' +
    '</div>' +
  ...
  if (selectAll) {
    selectAll.addEventListener('change', function() {
      document.querySelectorAll('.party-checkbox').forEach(function(cb) { cb.checked = selectAll.checked; });
    });
  }
}
...
function applyBulkInit() {
  ...
  document.querySelectorAll('.party-init-input').forEach(function(input) { input.value = val; });
}

function confirmAddFromPartyBulk() {
  var checkedItems = document.querySelectorAll('.party-checkbox:checked');
  if (checkedItems.length === 0) { alert('Zaznacz co najmniej jedną postać!'); return; }
  var added = 0;
  checkedItems.forEach(function(cb) {
    var idx = parseInt(cb.dataset.index);
    var player = players[idx];
    if (!player || player.hp <= 0) return;
    var exists = combatants.some(function(c) { return c.name === player.name && c.role === player.role; });
    if (exists) return;
    var initInput = document.querySelector('.party-init-input[data-index="' + idx + '"]');
    var initVal = initInput ? parseInt(initInput.value) || 0 : 0;
    addCombatant({
      name: player.name, init: initVal, hp: player.hp, maxHp: player.maxHp,
      ac: player.ac, role: player.role,
  ...
  if (added > 0) { playSound('add'); renderInit(); updateCombatStats(); }
  else alert('Nie dodano żadnej nowej postaci.');
}
...
function confirmAddCombatant() {
  ...
  addCombatant({ name: name, init: init, hp: hp, maxHp: hp || null, ac: ac || '—', role: role, avatar: '⚔️' });
...
function nextTurn() {
  ...
  if (isNewRound) {
    ...
    combatants.forEach(function(c) {
      if (c.timers && c.timers.length > 0) {
        c.timers = c.timers.filter(function(t) { t.remaining--; return t.remaining > 0; });
      }
    });
    focusFire = [];
    turnLog = [];
  ...
  if (current && current.status === 'dead') { nextTurn(); return; }
  if (current) processTurnStartEffects(current);
  renderInit();
  updateFocusFire();
  updateCombatStats();
  syncPlayerData();
...
function processTurnStartEffects(combatant) {
  if (combatant.effects && combatant.effects.length > 0) {
    ...
    toRemove.forEach(function(idx) { combatant.effects.splice(idx, 1); });
  }
}
...
function animateRoundChange() {
  ...
  entries.forEach(function(el, i) {
    setTimeout(function() {
      ...
      if (currentEntry && el === currentEntry) el.classList.add('round-change-current');
      else el.classList.add('round-change');
      setTimeout(function() { el.classList.remove('round-change', 'round-change-current'); }, 700);
    }, i * 40);
  });
}
...
function openTimerPopup() {
  if (combatants.length === 0) { alert('Brak bojowników w potyczce!'); return; }
  ...
  popup.id = 'timerPopup';
  var presetOptions = [
    { label: 'Koncentracja' }, { label: 'Oślepienie' }, { label: 'Ogłuszenie' },
    { label: 'Paraliż' }, { label: 'Trucizna' }, { label: 'Prędkość' },
    { label: 'Ochrona' }, { label: 'Płonący' }, { label: 'Zamrożony' }, { label: 'Przerażony' }
  ];
  var presetBtns = presetOptions.map(function(p) {
    return '<button class="timer-preset-btn" onclick="document.getElementById(\'timerNameInput\').value=\'' + p.label + '\'">' + p.label + '</button>';
  }).join('');
  ...
  popup.innerHTML = '<div class="popup-content timer-popup-content">' +
    ...
    '<div class="attack-row"><label>Cel</label><select id="timerTarget">' + targetOptions + '</select></div>' +
    '<div class="attack-row"><label>Nazwa efektu</label><input id="timerNameInput" placeholder="Nazwa efektu..." /></div>' +
    '<div class="timer-presets">' + presetBtns + '</div>' +
    '<div class="attack-row"><label>Czas trwania (tury)</label><input type="number" id="timerDuration" value="3" min="1" max="999" /></div>' +
...
function addTimerToCombatant() {
  ...
  else c.timers.push({ name: name, duration: duration, remaining: duration });
  ...
  combatants.forEach(function(c, i) {
    if (c.timers && c.timers.length > 0) {
      c.timers.forEach(function(t) {
        allTimers.push({ combatant: c.name, index: i, name: t.name, remaining: t.remaining });
      });
    }
  });
  if (allTimers.length === 0) {
    container.innerHTML = '<div style="color:var(--parchment-dim);font-size:.7rem;text-align:center;padding:6px;">Brak aktywnych timerów</div>';
    return;
  }
...
function openAttackPopup() {
  ...
  for (var i = 0; i < combatants.length; i++) {
    if (i !== defaultAttacker && combatants[i].status !== 'dead') {
      ...
      if (isPlayer !== isAttackerPlayer) { defaultTarget = i; break; }
    }
  }
  if (defaultTarget === -1) {
    for (var j = 0; j < combatants.length; j++) {
      if (j !== defaultAttacker && combatants[j].status !== 'dead') { defaultTarget = j; break; }
    }
  }
  var targetOptions = combatants.map(function(c, i) {
  ...
  popup.innerHTML = '<div class="popup-content attack-popup-content">' +
    '<div class="popup-title">⚔️ Atak</div>' +
    '<div class="attack-form">' +
      '<div class="attack-row"><label>Atakujący</label><select id="attackerSelect">' + attackerOptions + '</select></div>' +
      '<div class="attack-row"><label>Cel</label><select id="targetSelect">' + targetOptions + '</select></div>' +
      '<div class="attack-row"><label>Bonus do trafienia</label><input type="number" id="attackBonus" value="0" step="1" /></div>' +
      '<div class="attack-row"><label>Kości obrażeń (np. 2d6+3)</label><input type="text" id="damageDice" placeholder="np. 2d6+3" value="1d8" /></div>' +
      '<div class="attack-row"><label>Typ obrażeń</label>' +
        '<select id="damageType">' +
          '<option value="obuchowe">⚡ Obuchowe</option><option value="cięte">🗡️ Cięte</option>' +
          '<option value="kłute">🏹 Kłute</option><option value="ogniste">🔥 Ogniste</option>' +
          '<option value="zimne">❄️ Zimne</option><option value="elektryczne">⚡ Elektryczne</option>' +
          '<option value="trucizna">☠️ Trucizna</option><option value="nekrotyczne">💀 Nekrotyczne</option>' +
          '<option value="kwas">🧪 Kwas</option><option value="psychiczne">🧠 Psychiczne</option>' +
          '<option value="promieniowanie">☀️ Promieniowanie</option><option value="grzmot">🔊 Grzmot</option>' +
        '</select></div>' +
      '<div class="attack-check">' +
        '<input type="checkbox" id="attackAdvantage" /><label for="attackAdvantage">⚡ Przewaga</label>' +
        '<input type="checkbox" id="attackDisadvantage" /><label for="attackDisadvantage">🌑 Utrudnienie</label>' +
      '</div>' +
      '<div class="attack-actions">' +
...
function executeAttack() {
  ...
  if (attackerIdx === targetIdx) { alert('Nie możesz atakować samego siebie!'); return; }
  ...
  if (result) {
    ...
    if (container) {
      var resultHtml = '';
      if (result.miss) {
        resultHtml = '<div class="attack-result"><div class="attack-result-header">💨 PUDŁO!</div><div class="attack-result-detail">Atak chybił celu</div></div>';
      } else if (!result.hit) {
        resultHtml = '<div class="attack-result"><div class="attack-result-header">🛡️ NIETRAFIONY!</div><div class="attack-result-detail">' + (result.totalAttack || '?') + ' vs AC ' + combatants[targetIdx].ac + '</div></div>';
      } else {
        var critText = result.crit ? ' 💥 KRYTYK!' : '';
        var deathHtml = combatants[targetIdx].hp <= 0 ? '<div class="attack-result-death">💀 CEL PADA!</div>' : '';
        resultHtml = '<div class="attack-result">' +
          '<div class="attack-result-header">🎯 TRAFIONY!' + critText + '</div>' +
          '<div class="attack-result-detail">' + result.attackRoll + ' + ' + attackBonus + ' = ' + result.totalAttack + ' (AC ' + combatants[targetIdx].ac + ')<br>' +
          '🎲 ' + damageDice + ' → ' + result.damageRolls.join(' + ') + ' = ' + result.damage + ' ' + damageType + '</div>' +
...
function performAttack(attackerIndex, targetIndex, attackBonus, damageDice, damageType, advantage, disadvantage) {
  ...
  if (attackerIndex === targetIndex) { alert('Nie możesz atakować samego siebie!'); return null; }
  if (target.status === 'dead') { alert('Cel już nie żyje!'); return null; }
  var roll1 = rollDice(20);
  var roll2 = rollDice(20);
  var attackRoll;
  if (advantage && !disadvantage) attackRoll = Math.max(roll1, roll2);
  else if (disadvantage && !advantage) attackRoll = Math.min(roll1, roll2);
  else attackRoll = roll1;
  ...
  return {
    hit: true, crit: isCrit, miss: false,
    attackRoll: attackRoll, totalAttack: totalAttack,
    damage: damage, damageRolls: damageRolls, target: target.name
  };
}
...
function dealDamage(attackerIndex, targetIndex, damage, damageType, isCrit) {
  ...
  if (actualDamage > 0) {
    ...
    if (target.hp <= 0) {
      ...
      playSound('death');
      if (attacker) addTurnLog(attacker.name, '🏆 Zabił ' + targetName + '!');
    } else {
  ...
  if (existing) {
    ...
    focusFire.push({ name: targetName, dmg: actualDamage, status: '⚔️ ' + actualDamage + ' ' + damageType });
  }
  renderInit();
  updateFocusFire();
  updateCombatStats();
...
function updateFocusFire() {
  ...
  container.innerHTML = logHtml || '<div style="color:var(--parchment-dim);font-size:.6rem;text-align:center;padding:3px;">Brak akcji w tej rundzie</div>';
}
...
function showInitDmg(index) {
  if (typeof dmgPopupTarget !== 'undefined') dmgPopupTarget = { type: 'init', index: index };
  if (typeof showDamagePopup === 'function') showDamagePopup(combatants[index] ? combatants[index].name : 'Bojownik');
}
...
function showInitCondPopup(index) {
  ...
  if (typeof showCondPopup === 'function') {
    showCondPopup(c.name, c.conditions || [], c.exhaustionLevel || 0, function(cond, exhaustionLevel) {
      ...
      if (exhaustionLevel !== undefined) c.exhaustionLevel = Math.max(0, Math.min(6, exhaustionLevel));
      renderInit();
      syncPlayerData();
...
// ====== EFEKTY (dodane - były eksportowane ale brakowało definicji!) ======
function addEffect(combatantIndex, effectName, duration) {
  ...
  if (existing) existing.remaining = duration;
  else c.effects.push({ name: effectName, duration: duration, remaining: duration });
  addTurnLog(c.name, '⏱️ ' + effectName + ' (' + duration + ' tur)');
  renderInit();
  playSound('state');
}

function addEffectPopup(combatantIndex) {
  var c = combatants[combatantIndex];
  if (!c) return;
  var name = prompt('Nazwa efektu dla ' + c.name + ':');
  if (!name) return;
  var duration = parseInt(prompt('Czas trwania (tury):', '3')) || 3;
  addEffect(combatantIndex, name, duration);
}
...
function renderInit() {
  ...
  combatants.forEach(function(c, i) {
    ...
    if (i === combatants.length - 1 && lastInitCount !== combatants.length) div.classList.add('slide-in');
...
function renderInitEntry(div, c, i) {
  ...
  if (c.exhaustionLevel > 0) {
    ...
    exhaustionTag = '<span class="cond-tag" style="background:rgba(255,107,107,0.15);border-color:rgba(255,107,107,0.2);color:#ff6b6b;">🥱 Wyczerpanie ' + exLevel + '/6</span>';
  }
  var effectsTags = '';
  ...
  var statusIcon = c.status === 'dead' ? '💀 ' : '';
  var hasCondClass = c.conditions && c.conditions.length > 0 ? 'has-cond' : '';
  var condBtnIcon = c.conditions && c.conditions.length > 0 ? '🔄' : '⚙️';
  var acHtml = c.ac && c.ac !== '—' ? '<div class="init-ac" title="Klasa Pancerza">🛡️ ' + c.ac + '</div>' : '';
  var hpHtml = typeof c.hp === 'number' ? '<div class="init-hp ' + hpClass + '" onclick="event.stopPropagation();showInitDmg(' + i + ')">' + hpText + '</div>' : '';
  var tempHtml = c.tempHp > 0 ? '<div class="init-temp" title="Tymczasowe HP">🛡️+' + c.tempHp + '</div>' : '';
  var effectBtn = c.status !== 'dead' ? '<button class="init-cond-btn" onclick="event.stopPropagation();addEffectPopup(' + i + ')" title="Dodaj efekt">⏱️</button>' : '';
  
  div.innerHTML = 
    '<div class="init-badge">' + c.init + '</div>' +
    '<div class="init-name">' + (i === currentTurn ? '▶ ' : '') + statusIcon + c.name + ' ' + condTags + exhaustionTag + effectsTags + timerTags + dmgCounter +
      '<button class="init-cond-btn ' + hasCondClass + '" onclick="event.stopPropagation();showInitCondPopup(' + i + ')" title="Zarządzaj stanami">' + condBtnIcon + '</button>' +
      '<button class="init-cond-btn" onclick="event.stopPropagation();showInitDmg(' + i + ')" title="Zadaj obrażenia">⚔️</button>' +
      effectBtn +
    '</div>' +
    acHtml + hpHtml + tempHtml +
...
function updateRoundBadge() {
  ...
  if (isNewRound && round > 1) animateRoundNumber();
}

function animateRoundNumber() {
  var numSpan = document.querySelector('.round-number');
  if (numSpan) { numSpan.classList.remove('bump'); void numSpan.offsetWidth; numSpan.classList.add('bump'); }
}
...
function scrollToCurrentTurn() {
  ...
  if (isMobile) {
    ...
    if (current) setTimeout(function() { current.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' }); }, 150);
  }
}
...
function resetInit() {
  ...
  turnLog = [];
  combatActive = false;
  combatStats = { totalRounds: 0, totalDamage: 0, kills: 0, crits: 0, misses: 0, startTime: null, endTime: null };
  renderInit();
  updateFocusFire();
  updateCombatStats();
  syncPlayerData();
