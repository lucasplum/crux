// ============================================================
//  GRIMOIRE - NOWA KSIĘGA ZAKLĘĆ (kafelki, filtry, modal)
// ============================================================
...
// MAPA odwrotna (nazwa szkoły → klucz)
var GRIMOIRE_SCHOOL_KEYS = {};
for (var key in GRIMOIRE_SCHOOLS) {
  GRIMOIRE_SCHOOL_KEYS[key] = key;
}
...
function loadGrimoireSpells(callback) {
  ...
  levels.forEach(function(level) {
    // Poprawione ścieżki – względem głównego katalogu
    var urls = [
    ...
    tryLoadGrimoireUrl(urls, 0, function(data) {
      if (data && Array.isArray(data)) {
        // Normalizacja danych do formatu Grimuar
        data.forEach(function(s) {
          var schoolKey = s.school || '';
          // Mapowanie nazw szkół na klucze
          var schoolMap = {
          ...
          var schoolName = schoolMap[schoolKey.toLowerCase()] || schoolKey || 'Nieznana';
...
function tryLoadGrimoireUrl(urls, idx, callback) {
  ...
  fetch(urls[idx])
    ...
    .catch(function() {
      console.warn('Nie udało się załadować:', urls[idx]);
  ...
  box.innerHTML = `
    ...
    <div class="modal-body">
      ...
      <div class="desc-section">
        ...
        <div class="desc-en">
          ${spell.descEn}
          ${spell.materialNoteEn ? '<p style="margin-top:12px;font-size:0.75rem;"><em>' + spell.materialNoteEn + '</em></p>' : ''}
        </div>
      </div>
...
// ---------- FILTRY (Custom Select) ----------
function initGrimoireSelects() {
  // Wypełnij opcje szkół
...
// Auto-start po DOMContentLoaded
